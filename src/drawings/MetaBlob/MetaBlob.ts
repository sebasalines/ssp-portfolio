...
export class MetaBlob {
  ...
  constructor(opts: MetaBlobOptions) {
    ...
    this.size = opts.size
    this.metaBalls = new Paper.Group()
    ...
    this.originals = new Paper.Group({
      parent: this.metaBalls,
    })
    ...
    // colours
    this.planetColour = new Paper.Color(opts.color || 'black')
    this.moonColour = new Paper.Color(opts.color || 'black')
    this.connectorColour = new Paper.Color(opts.color || 'black')
    this.circles = []
    this.numberOfCircles = Math.ceil(Math.random() * 5) + 4
    ...
    this.center = opts.center || Paper.view.center
    ...
    for (var i = 0; i < this.numberOfCircles; i++) {
      ...
      var circle = new Paper.Path.Circle({
        ...
        moon: isMoon,
        parentPlanet: i - 1,
        fillColor: isMoon ? this.moonColour : this.planetColour,
  ...
  createCircleOffset(circle) {
    ...
    // for planet
    return new Paper.Point(
      Math.floor(circle.offsetRandomSeed[0] * this.offSetXRandom * 0.25) + this.offSetXMin * 0.5,
      Math.floor(circle.offsetRandomSeed[1] * this.offSetYRandom * 0.25) + this.offSetYMin * 0.5,
    )
  }

  move(count) {
    for (let i = 0; i < this.circles.length; i++) {
      const element = this.circles[i];
  ...
  moveCircles(count, i) {
    ...
    var offsetX = this.circles[i].moon
      ? this.circles[i].offset.x / this.offsetXVariant
      : this.circles[i].offset.x
    var offsetY = this.circles[i].moon
      ? this.circles[i].offset.y / this.offsetYVariant
      : this.circles[i].offset.y
    var sin0 = this.circles[i].moon
      ? this.circles[i].sine[0] / this.sineVariant
      : this.circles[i].sine[0]
    var sin1 = this.circles[i].moon
      ? this.circles[i].sine[1] / this.sineVariant
      : this.circles[i].sine[1]
    ...
    spinX += deltaX
    spinY += deltaY
    var point = new Paper.Point(spinX, spinY)
